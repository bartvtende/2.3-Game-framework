...
/**
 * JPanel that holds all button involved in choosing what player is played
 * against.
 * 
...
public class PlayersController extends JPanel {
	...
	/**
	 * This method makes an JList out of the players available in the array
	 * playersList.
	 */
	...
	/**
	 * Creates the button for connecting to the specified player and the actions
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if (playerList.getSelectedValue() != null) {
					...
					JOptionPane.showMessageDialog(MainView.mainview,
							"You didn't select a player, please try again.",
							"Select a player!", JOptionPane.ERROR_MESSAGE);
				}
				...
				if (MainView.games.getSelectedGame() != null) {
					...
					JOptionPane.showMessageDialog(MainView.mainview,
							"You didn't select a game, please try again.",
							"Select a game!", JOptionPane.ERROR_MESSAGE);
				}
				...
				if (MainView.games.getPlayerType() == null) {
					JOptionPane
							.showMessageDialog(
									MainView.mainview,
									"You didn't select a player type, please try again.",
	...
	/**
	 * Creates the button for refreshing the JList with the player names and the
	 * actions that will be performed when it is clicked.
	 */
	...
	/**
	 * Sets the contents of the JList to that of the array that is passed as a
	 * parameter.
	 * 
	 * @param players
	 *            The array that contains the player names.
	 */
